    ...
    <div className="space-y-6">
      <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-[#64748b] mt-1">Gestion de silabos por materia y periodo</p>
        </div>
            ...
            <TableBody>
              {filtered.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell className="text-sm text-[#64748b]">{materiasMock.find((m) => m.id === s.materiaId)?.nombre ?? `Materia ${s.materiaId}`}</TableCell>
                  <TableCell className="text-sm text-[#64748b]">{periodosMock.find((p) => p.id === s.periodoId)?.nombre ?? `Periodo ${s.periodoId}`}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      {s.estado === "pendiente" && (
                        <>
                          <Button variant="ghost" size="sm" onClick={() => handleEstado(s.id, "aprobado")} className="text-[#1a6b3c] hover:bg-[#e8f5ee]"><CheckCircle2 className="w-4 h-4" /></Button>
