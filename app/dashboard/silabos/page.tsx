...
import { Plus, Pencil, Trash2, Search, FilePlus, CheckCircle, Clock, XCircle } from "lucide-react"
...
const empty: Omit<InformeDocencia, "id"> = {
  docente: "", materiaId: 1, periodoId: 1, tipo: "silabo",
...
export default function SilabosPage() {
  ...
  useEffect(() => {
    DocenciaService.getAll().then((all) => setInformes(all.filter((i) => i.tipo === "silabo")))
    MateriaService.getAll().then(setMaterias)
  }, [])

  const filtered = informes.filter((i) =>
    i.docente.toLowerCase().includes(search.toLowerCase())
  )

  const openCreate = () => { setEditing(null); setForm(empty); setOpen(true) }
  const openEdit = (i: InformeDocencia) => {
    setEditing(i)
    setForm({ docente: i.docente, materiaId: i.materiaId, periodoId: i.periodoId, tipo: "silabo", fechaEntrega: i.fechaEntrega, estado: i.estado })
    setOpen(true)
  }

  const handleSave = async () => {
    if (!form.docente || !form.fechaEntrega) {
      toast.error("Complete los campos obligatorios"); return
    }
    if (editing) {
      const updated = await DocenciaService.update(editing.id, form)
      if (updated) { setInformes((prev) => prev.map((i) => i.id === editing.id ? updated : i)); toast.success("Silabo actualizado") }
    } else {
      const created = await DocenciaService.create(form)
      setInformes((prev) => [created, ...prev]); toast.success("Silabo registrado")
    }
    setOpen(false)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#0f172a]">Mis Silabos</h1>
          <p className="text-[#64748b] mt-1">Registro y seguimiento de silabos de asignatura</p>
        </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? "Editar silabo" : "Registrar silabo"}</DialogTitle>
          </DialogHeader>
